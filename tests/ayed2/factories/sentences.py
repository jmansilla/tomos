...
class VarDeclarationFactory(factory.Factory):
    ...
    class Params:
        faker_type = factory.Faker("random_element", elements=type_registry.type_map.values())
...
class IfFactory(factory.Factory):
    ...
    then_sentences = factory.List([factory.SubFactory(AssignmentFactory) for _ in range(3)])
    else_sentences = factory.List([factory.SubFactory(AssignmentFactory) for _ in range(3)])
...
class WhileFactory(factory.Factory):
    ...
    guard = factory.SubFactory("tests.ayed2.factories.expressions.BooleanLiteralFactory")
    sentences = factory.List([factory.SubFactory(AssignmentFactory) for _ in range(3)])
